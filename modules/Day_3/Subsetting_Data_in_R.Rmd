...
```{r}
select(tb, starts_with("c"), ends_with("r"))
...
filter(tb, "mpg!" > 22) # will not work correctly
```
...
```{r}
filter(tb, "car" == "Ferrari Dino") # this will not work
```

## Be careful with column names and `filter`

```{r}
filter(tb, car == "Ferrari Dino") # this works!
```
