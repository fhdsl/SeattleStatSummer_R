...
```{r}
head(mtcars) # looking at the top of the mtcars data
mtcars_og <- mtcars
```
...
```{r, eval = FALSE}
mtcars <- rename(mtcars, MPG! = MPG) # this will cause an error
```
...
```{r}
mtcars_upper <- rename_with(mtcars, toupper)
head(mtcars_upper, 3)
```

```{r}
mtcars <- rename_with(mtcars, tolower)
head(mtcars, 3)
```
